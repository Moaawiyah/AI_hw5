"""Baseline runner: load model directly on the local accelerator (§5.2).

Expected on an accelerator smaller than the 28 GB FP16 model: OOM.
...
def _load(size_key: str, device: str):
...
def run(size_key: str, prompt: str, max_new_tokens: int) -> dict:
    ...
    rec["device"] = device
    try:
        tok, model = _load(size_key, device)
